...
class OnlinePromptRidge:
    ...
    def predict(self, features, prompt_ids) -> np.ndarray:
        ...
        predictions = np.einsum("nf,nf->n", features, self.weights[prompt_ids])
        ready = self.observations[prompt_ids] >= self.min_observations
        predictions = np.where(ready, predictions, 0.0)
        return np.clip(
    ...
    def update(self, features, prompt_ids, targets) -> None:
        ...
        for prompt_id in np.unique(prompt_ids):
            ...
            replay_size = int(
                min(self.observations[prompt_id], self.replay_capacity)
            )
            replay_x = self.replay_features[prompt_id, :replay_size].astype(
                np.float64
            )
            replay_y = self.replay_targets[prompt_id, :replay_size].astype(
                np.float64
            )
            kernel = replay_x @ replay_x.T
            kernel.flat[:: replay_size + 1] += self.ridge
            dual_weights = np.linalg.solve(kernel, replay_y)
            self.weights[prompt_id] = (replay_x.T @ dual_weights).astype(
                np.float32
            )
