...
TRAIN_D="${TRAIN_D:-8}"
SEED="${SEED:-0}"
TIME_BUDGET_SECONDS="${TIME_BUDGET_SECONDS:-${TIME_BUDGET:-7200}}"
...
RUN_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/outputs/qwen35_countdown_predictive_preview_D${TRAIN_D}_seed${SEED}_${RUN_TAG}}"
MASTER_LOG="${MASTER_LOG:-$REPO_ROOT/runs/qwen35_predictive_preview_D${TRAIN_D}_2h_master.log}"
...
echo "Preview-predicted EGGROLL"
...
echo "Use learned predictions: $PREDICTIVE_USE_PREDICTIONS"
...
"$VENV_PYTHON" -m llm_experiments.general_do_evolution \
  ...
  --predictive-min-observations "${PREDICTIVE_MIN_OBSERVATIONS:-256}" \
  ...
  --predictive-max-residual-ratio "${PREDICTIVE_MAX_RESIDUAL_RATIO:-0.9}" \
  --predictive-min-quality-nonzero-labels "${PREDICTIVE_MIN_QUALITY_NONZERO_LABELS:-4}" \
  "${PREDICTION_FLAGS[@]}" \
  --sigma 1e-3 \
  --lr-scale 0.2 \
  --seed "$SEED" \
  ...
  --wandb-name "predictive_preview_D${TRAIN_D}_seed${SEED}_2h" \
  2>&1 | tee "$MASTER_LOG"
