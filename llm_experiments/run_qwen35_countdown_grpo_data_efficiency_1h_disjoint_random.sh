...
# GRPO countdown_chat data-efficiency: disjoint train/val, random 8 prompts/epoch, 1h/GPU each.
...
run_case() {
  local train_d="$1"
  local log="$LOG_DIR/qwen35_countdown_grpo_D${train_d}_1h_disjoint_random.log"
  echo "=== Starting GRPO D${train_d} disjoint+random (${TIME_BUDGET}s) -> $log ==="
  "$VENV_PYTHON" -u -m llm_experiments.do_grpo \
    ...
    2>&1 | tee "$log"
  echo "=== Finished GRPO D${train_d} disjoint+random (exit $?) ==="
}

run_case 256
run_case 10

echo "Both GRPO disjoint+random 1h runs complete."
