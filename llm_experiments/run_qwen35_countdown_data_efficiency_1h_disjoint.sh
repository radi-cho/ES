...
# Qwen3.5-2B countdown_chat data-efficiency experiment (disjoint train/val split).
# Train D=256 vs D=10, fixed val D=256 (zero overlap), 1h budget each, EGGROLL.
...
run_case() {
  local train_d="$1"
  local log="$LOG_DIR/qwen35_countdown_D${train_d}_1h_disjoint.log"
  echo "=== Starting D${train_d} disjoint (${TIME_BUDGET}s budget) -> $log ==="
  "$VENV_PYTHON" -m llm_experiments.general_do_evolution \
    ...
    --wandb-name "D${train_d}_1h_disjoint" \
    2>&1 | tee "$log"
  echo "=== Finished D${train_d} disjoint (exit $?) ==="
}

run_case 256
run_case 10

echo "Both disjoint 1h runs complete."
