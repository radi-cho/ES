...
# EGGROLL D=30, 3h budget, disjoint val, random 8 train prompts/epoch from full pool.
...
LOG="$LOG_DIR/qwen35_countdown_eggroll_D30_3h_disjoint_random_full.log"
echo "=== EGGROLL D30 full pool disjoint+random (${TIME_BUDGET}s) -> $LOG ==="
...
"$VENV_PYTHON" -u -m llm_experiments.general_do_evolution \
  ...
  --wandb-name "D30_3h_disjoint_rand_full" \
  2>&1 | tee "$LOG"

echo "=== Finished EGGROLL D30 full pool (exit $?) ==="
