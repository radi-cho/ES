...
# D30 full pool, 3h, log val + train fitness on all 30 + update norms.
...
LOG="$LOG_DIR/qwen35_countdown_eggroll_D30_3h_metrics_disjoint_random_full.log"
echo "=== EGGROLL D30 metrics full (${TIME_BUDGET}s) -> $LOG ==="
...
"$VENV_PYTHON" -u -m llm_experiments.general_do_evolution \
  ...
  --wandb-name "D30_3h_metrics_full_disjoint_rand" \
  2>&1 | tee "$LOG"

echo "=== Finished D30 metrics full (exit $?) ==="
